...
Kp = 1000  # speed propotional gain
Lf = par.Lf  # look-ahead distance
T = par.TotalTime  # max simulation time
goal_dis = par.goal_dis
stop_speed = 0

#  animation = True
animation = False

class States:# 定义一个类，用于调用车辆状态信息

    def __init__(self, x=None, y=None, yaw=None, v=None, t = None):
        self.x = x
        self.y = y
        self.yaw = yaw
        self.v = v
...
def closed_loop_prediction(x_start, y_start, yaw_start, cx, cy, cyaw, speed_profile, goal):

    state = unicycle_model.State(x=x_start, y=y_start, yaw=yaw_start, v=0.0)
    ...
    while T >= time:
        ...
        if lastIndex == target_ind: break
        target_speed = speed_profile[target_ind]

        state = unicycle_model.update(state, target_speed, di) #target speed为当前时间速度，di为当前的转动角度
