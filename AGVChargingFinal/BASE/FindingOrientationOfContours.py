...
class FindingOrientationOfContours(object):
    ...
    def FindOrientation(self, contourA, contourB, contourC):
        ...
        of # TODO: the triangle formed by three contours and then we find the
    ...
    def findOrientationBetweenPoints(self, DistancePair, PairA, PairB):
        ...
        if (slope >= 0) and (distance >= 0):
            ...
            rotationAngle = self.__findatan(slope)
        ...
        #      回            tr
            ...
            rotationAngle = 180 + self.__findatan(slope)
        ...
        # 回    回      tr    tl
            ...
            rotationAngle = 360 +  self.__findatan(slope)
        # 回    回    tl   tr
        #
        # 回          bl
        elif (slope < 0) and (distance < 0):
        ...
        return rotationAngle, massQuad, ORIENTATION
    ...
    def __findCrossProduct(self, pointA, pointB):
        vec = np.array([pointB[0]-pointA[0], pointB[1]-pointA[1]])
        vec0 = np.array([1, 1])
        return np.cross(vec0, vec) >= 0
    
    def __findZero(self, pointA, pointB):
        vec = np.array([pointB[0]-pointA[0], pointB[1]-pointA[1]])
        return vec[0] > 0

    def __findatan(self, ratio):
        return math.atan(ratio) / math.pi * 180
class Pair(object):
