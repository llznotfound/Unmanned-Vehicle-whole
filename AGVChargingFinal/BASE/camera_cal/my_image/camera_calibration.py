...
def calibrate(drawconer=False):
    ...
    images = glob.glob(os.path.dirname(os.path.realpath(__file__)) +'/WechatIMG*.jpeg')

    print(os.path.dirname(os.path.realpath(__file__)) +'/WechatIMG*.jpeg')
    ...
    pickle_file = open(os.path.dirname(os.path.realpath(__file__)) +"/camera_cal.p", "wb")
...
def test():
    ...
    loc = os.path.dirname(os.path.realpath(__file__)) + "/WechatIMG15.jpeg"
    img = cv2.imread(loc)
    img_size = (img.shape[1], img.shape[0])
    newcameramtx, roi = cv2.getOptimalNewCameraMatrix(mtx, dist, img_size, 0,img_size)

    # dst = cv2.undistort(img, mtx, dist, None, mtx)
    mapx, mapy = cv2.initUndistortRectifyMap(mtx, dist, None, newcameramtx, img_size, 5)
