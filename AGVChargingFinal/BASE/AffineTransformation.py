...
class AffineTransformation(object):
    ...
    def Calangle(self, rotationAngle, massQuad):
        cv.putText(self.OriginalImage,
                    "angle is: " + str(rotationAngle), (30, 50), cv.FONT_HERSHEY_PLAIN, 1.5, (0, 255, 0), 1, 8, 0)
        cv.circle(self.OriginalImage, (massQuad.tr[0], massQuad.tr[1]), 5, (0, 255, 0), - 1)
        cv.circle(self.OriginalImage, (massQuad.tl[0], massQuad.tl[1]), 5, (255, 0, 0), - 1)
        cv.circle(self.OriginalImage, (massQuad.bl[0], massQuad.bl[1]), 5, (0, 0, 255), - 1)
        cv.putText(self.OriginalImage,
                    "x is: " + str(np.int32(massQuad.tr[0]+massQuad.bl[0])/2), (30, 70), cv.FONT_HERSHEY_PLAIN, 1.5, (0, 255, 0), 1, 8, 0)
        cv.putText(self.OriginalImage,
                   "y is: " + str(np.int32(massQuad.tr[1] + massQuad.bl[1]) / 2), (30, 70), cv.FONT_HERSHEY_PLAIN, 1.5,
                   (0, 255, 0), 1, 8, 0)
        cv.putText(self.OriginalImage,
                   "angle is: " + str(rotationAngle), (30, 50), cv.FONT_HERSHEY_PLAIN, 1.5, (0, 255, 0), 1, 8, 0)
